...
print("Max Value of Action ->  ", env.action_space.high)
print("Min Value of Action ->  ", env.action_space.low)
...
class Buffer:
	...
	def update(
		...
		with tf.GradientTape() as tape:
			...
			y = tf.clip_by_value(y, min_reward, 0)
...
def policy(state, std):
	...
	legal_action = np.clip(sampled_actions, env.action_space.low, env.action_space.high)
...
critic_optimizer = tf.keras.optimizers.Adam(critic_lr,  clipnorm = clip_norm)
actor_optimizer = tf.keras.optimizers.Adam(actor_lr,  clipnorm = clip_norm)
...
avg_reward_list = []
# To store average distance to the GOAL
dist_list = []

def normalize(state):

	s = np.array([state])
	scaler.partial_fit(s)
...
for ep in range(total_episodes):
	...
	while True:
		...
		reward  = get_reward(prev_state,action,state,Goal)
		
		buffer.record((normalized_prev_state, action, reward, normalized_state))
		buffer_accumulator.append((prev_state, action, reward, state))
		...
		if done:
			...
			for eg in extra_goals:
				...
				for ex in buffer_accumulator:
					...
					new_experience = (s,a,r,ss)
					buffer.record(new_experience)
			...
			if ep%1000 ==0:
				...
				pickle.dump(ep_reward_list, open('ep_reward_list.pkl', 'wb'))
				pickle.dump(dist_list, open('dis_list.pkl','wb'))
	...
	if ep%100==0:
		avg_dist = np.mean(dist_list[-100:])
		print('==================================================')
		print("EPOCH =  ", ep//(800))
